...
public class PlayPauseButton extends View {
  ...
  /** 座標の基準を ・中心を(0,0) ・右上(1,1) 右下(1,ー1) 左上(ー1,1) 左下(ー1,ー1) の座標時期に変換するためのClass */
  ...
  /** ボタンの色 */
  private int mBackgroundColor = getResources().getColor(R.color.colorAccent);
  ...
  /** コンストラクタ android:backgroundのデータを取得しボタンの色とする {@inheritDoc} */
  ...
  /** Animatorの初期化 それぞれのAnimatorに初期値をセットし､startさせている */
  ...
  protected void onDraw(Canvas canvas) {
    ...
    mRightPath.moveTo(
        ...
        mPoint.getY((Float) mCenterEdgeAnimator.getAnimatedValue()));
    mRightPath.lineTo(
        mPoint.getX(0.5 * SQRT_3), mPoint.getY((Float) mRightEdgeAnimator.getAnimatedValue()));
    mRightPath.lineTo(
        mPoint.getX(0.5 * SQRT_3), mPoint.getY(-1 * (Float) mRightEdgeAnimator.getAnimatedValue()));
    mRightPath.lineTo(
        mPoint.getY(-1 * (Float) mLeftEdgeAnimator.getAnimatedValue()),
        mPoint.getY(-1 * (Float) mCenterEdgeAnimator.getAnimatedValue()));
  ...
  /** 回転状況を {@link PlayPauseButton.SavedState} に保存する {@inheritDoc} */
  @Override
  ...
  /** Restoreしてきたやつから {@link PlayPauseButton.SavedState} を取得し状況をセットする {@inheritDoc} */
  @Override
  ...
  /** android:background で親の {@link View} が背景をセットするので上書き {@inheritDoc} */
  @Override
